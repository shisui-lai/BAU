...
const CSV_BUFFER_INTERVAL = 1000 // 缓冲刷盘间隔
const HEADER =
  [
    'ID',
    '时间',
...
async function appendToCSV(lines, ip = null) {
  ...
  try {
    ...
    // 监控文件大小（仅在接近阈值时）
    //await logFileSize(filePath, `IP ${ip || 'unknown'}`)
    ...
    try {
      if (needHeader) {
        await appendFileWithRetry(filePath, BOM_HEADER) // 如果需要表头，写入表头
      }

      const dataToWrite = lines + '\r\n'
      await appendFileWithRetry(filePath, dataToWrite) // 写入数据

      if (ip) {
        /* console.log(
          `[报文导出] IP ${ip} 成功写入 ${lines.split('\r\n').length} 行数据到: ${filePath}`
        ) */
      }
    } finally {
      // 在写入完成后解锁文件
    ...
    try {
      ...
      console.error('csvMutex release异常', e)
    }
  }
}
export {
