...
function paraseAllowChrgPCS(param, status) {
  switch (param) {
    ...
    case 1:
      return (status = 'discharge' ? '禁止充电' : '禁止放电')
    default:
      return `未知${param}` // 添加默认值处理
  }
}
...
function paraseAlarmPCSSYL(param, status) {
  switch (param) {
    ...
    case 1:
      return (status = 'fault' ? '有一级或二级报警' : '有三级报警')
    default:
      return `未知${param}` // 添加默认值处理
  }
}
...
function getData_ClusterSummNew(params, bmuCount, withUnit) {
    ...
    },
    {
      ...
      element: [
        ...
        },
        {
          id: 208,
          label: '系统总状态位',
          value: parseSystemTotalFaultBits(params[15], params[16])
        }
      ]
    },
    {
  ...
  try {
    ...
    if (params.length >= totalProductRegs) {
      const productStartIndex = params.length - totalProductRegs
      for (let i = 0; i < bmuCount; i++) {
        ...
        const codeStr = convertToAscii_ClusterSummNew(sliceRegs)
...
function getAlarmData2(
  ...
  return faultConfigs.reduce((result, { name, addr, cells }) => {
    // 计算每个BMU需要的寄存器数量（每8个单体占用1个寄存器）
    const regsPerBMU = Math.ceil(cells / 8)

    // 计算该故障类型总共需要的寄存器数量
    const totalRegs = config.bmuTotal * regsPerBMU

    // 遍历所有寄存器，按单体连续解析
    for (let regIdx = 0; regIdx < totalRegs; regIdx++) {
      if (regIdx >= data.length) {
        console.warn(`地址溢出: ${name} 寄存器[${regIdx}]`)
        continue
      }

      const regValue = data[regIdx] || 0

      // 计算当前寄存器对应的BMU编号和该BMU内的寄存器索引
      const bmu = Math.floor(regIdx / regsPerBMU) + 1
      const localRegIdx = regIdx % regsPerBMU

      // 计算当前寄存器对应的起始单体编号（在该BMU内）
      const baseCell = localRegIdx * 8 + 1

      // 按位解析（从低位到高位，每2个bit代表一个单体）
      for (let bitPos = 0; bitPos <= 14; bitPos += 2) {
        const localCellNum = baseCell + bitPos / 2

        // 检查是否超出该BMU的单体数量
        if (localCellNum > cells) break

        // 计算全局连续序号（按单体连续排列，不考虑BMU边界）
        const globalCellNum = (bmu - 1) * cells + localCellNum
...
function convertDisconnectToAlarmFormat(disconnectData) {
      ...
      element: category.element
        .map((item) => {
          ...
          const isFault =
            item.value === '掉线' ||
            item.value === '失联' ||
            (item.value !== '正常' && item.label.includes('断连位置'))
          // 构建故障名称
          ...
          return {
            ...
            bmuIndex: extractBMU(item.label),
            cellIndex: extractCell(item.label),
            timestamp: Date.now()
          }
        })
...
function parseOtherFaults(params) {
  const errorMap = []
  // 解析寄存器数据
  const regA03E = params[53] // 0xA03E寄存器
  const faultsA03E = [
    '单体电池压差过大',
    '单体电池温差过大',
    'SOC差异过大',
  ...
  const regA03F = params[54] // 0xA03F寄存器
  const faultsA03F = [
    '充电过流',
    '放电过流',
    'RT1过温',
...
function getAlarmData1(params, bmuTotal) {
  const errorMap = [
    ...parseHardwareFaults(params, bmuTotal),
    /*  ...parseAlarmA016_A025_D000_D00b(params), */
    ...parseBMU2BitFault(params, 29, 'BMU过压', bmuTotal),
    ...parseBMU2BitFault(params, 33, 'BMU欠压', bmuTotal),
    ...parseBMU2BitFault(params, 37, 'BMU过温', bmuTotal),
    ...parseBMU2BitFault(params, 41, 'BMU欠温', bmuTotal),
    ...parseBMU2BitFault(params, 44, '1号动力接插件过温', bmuTotal),
    ...parseBMU2BitFault(params, 48, '2号动力接插件过温', bmuTotal),
...
function regNumComb_Float(reg1Value, reg2Value, isBigEndian = true) {
  if (typeof reg1Value !== 'number' || typeof reg2Value !== 'number') {
    throw new Error('reg1Value 和 reg2Value 必须是数字')
  }
  ...
  // 根据字节序写入寄存器值
  if (isBigEndian) {
    // 大端序：寄存器1 = 高16位，寄存器2 = 低16位
    view.setUint16(0, reg1Value, false) // 高地址存储高字节
    view.setUint16(2, reg2Value, false) // 低地址存储低字节
  } else {
    // 小端序：寄存器1 = 低16位，寄存器2 = 高16位
    view.setUint16(0, reg1Value, false) // 低16位放在低地址
    view.setUint16(2, reg2Value, false) // 高16位放在高地址
  }
  /*   console.log(view.getFloat32(0, isBigEndian)) */
  // 读取为 IEEE 754 单精度浮点数
  return view.getFloat32(0, isBigEndian) // 第二个参数表示字节序 (false = 大端序)
}

// 反向函数：将32位float拆解为两个16位寄存器值
function floatToRegisters(floatValue, isBigEndian = true) {
  ...
  // 将float写入缓冲区
  view.setFloat32(0, floatValue, isBigEndian)

  // 根据字节序读取两个16位寄存器值
  if (isBigEndian) {
    // 大端序：寄存器1 = 高16位，寄存器2 = 低16位
    return {
      reg1: view.getUint16(0, false), // 高16位
      reg2: view.getUint16(2, false) // 低16位
    }
  } else {
    // 小端序：寄存器1 = 低16位，寄存器2 = 高16位
    return {
      reg1: view.getUint16(0, false), // 低16位
      reg2: view.getUint16(2, false) // 高16位
    }
  }
}
...
function getArray_ConfigSOX1(params) {
  ...
  let valueArray = [
    ...
    regNumComb_Float(params[102], params[103], false).toFixed(1), //SOH计算-充电累积安时
    regNumComb_Float(params[104], params[105], false).toFixed(1), //SOH计算-放电累积安时
...
function parseKeHuaPCSData0(data, flag) {
  // 直接用位运算提取bit4~bit6和bit12~bit15
  const dec4to6 = (data >> 4) & 0b111
  const dec12to15 = (data >> 12) & 0b1111
  if (flag === 1) {
    switch (dec4to6) {
      case 0:
        return '初始化'
  ...
  } else if (flag === 0) return dec12to15
}
...
function parsePCSData(data, type) {
  ...
  const values = {
    ...
    keHuaPCS: [
      parseKeHuaPCSData0(data[0], 1),
      parseKeHuaPCSData0(data[0], 0),
      data[1].toString(),
      transToUnsigned(data[2], 'unsigned', 0.1),
      transToUnsigned(data[3], 'unsigned', 0.1) - 2000,
  ...
  const transKeyLabels = {
    ...
    keHuaPCS: Array.from({ length: labels.keHuaPCS.length }, (_, i) => `keHuaPCSLabel${i + 1}`)
  }
...
function parseRefrigerationData(data, type) {
  ...
  const values = {
    ...
    evk: [
      ...
      ...data.slice(27, 30).map(parseEvkFault1),
...
function parseDehumData(data, type) {
  ...
  let transKeyLabels = {
    dehum1: Array.from({ length: 32 }, (_, i) => `dehum1Label${i + 1}`)
  }
  let labels = {
    dehum1: [
      '雾化模块状态',
  ...
  let values = {
    dehum1: [
      ...
      data[7],
      data[8],
      ...Array.from({ length: 7 }, (_, i) => transToUnsigned(data[i + 9], 'unsigned', 0.1))
    ]
  }
  // Loop to add labels and values based on type
  let dataLabels, dataValues, keyLabels
...
function parseFireData(data, type, bmuNum) {
  ...
  const values = [
    ...
    parseFireData1_(data[2], 1, 1, bmuNum),
