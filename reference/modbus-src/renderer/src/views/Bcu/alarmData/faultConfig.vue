...
// 故障配置数据
const faultConfigData = shallowRef({
  faultMap: [],
  totalFault: {},
  diDOStatus: {}
})
...
<template>
  <div class="card">
    ...
    >
      ...
      <Column field="label" :header="t(`alarm.totalFaults.titles.faults`)">
        <template #body="{ data }">
    ...
    <div v-else-if="selectedDataType === 'totalFault'" class="fault-tables-container">
      ...
      >
        ...
        <DataTable :value="faults" showGridlines class="centered-table">
          <template #header>
            <div style="text-align: left">
              {{ t(`alarm.totalFaults.titles.${category}`) }} ({{ faults.length }})
            </div>
          </template>
          <Column field="label" :header="t('alarm.title3')">
            <template #body="{ data }">
              {{ t(`alarm.totalFaults.${data.label}`, data.label) }}
            </template>
          </Column>
          <Column field="value" :header="t('alarm.title6')" sortable>
            <template #body="{ data }">
              <span
              ...
              <span
                v-else-if="
                  data.value === '轻微' || (data.value === '无故障') | (data.value === '有效')
                "
                class="level-minor"
                >{{ t(`alarm.levels.${data.value}`) }}</span
              >
