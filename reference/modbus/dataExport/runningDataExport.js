...
// ✅ 内存优化：限制告警缓存大小
const MAX_STORED_ALARMS = 3000 // 最多存储3000个告警（从5000降低）
const ALARM_CLEANUP_INTERVAL = 300000 // 每5分钟清理一次
let lastAlarmCleanup = Date.now()

// 清理过期告警缓存
function cleanupStoredAlarms() {
  const now = Date.now()
  if (now - lastAlarmCleanup < ALARM_CLEANUP_INTERVAL) {
    return
  }
  
  lastAlarmCleanup = now
  
  if (storedAlarms.size > MAX_STORED_ALARMS) {
    // 清理最旧的1/3条目
    const entriesToDelete = Math.floor(storedAlarms.size / 3)
    const keys = Array.from(storedAlarms.keys()).slice(0, entriesToDelete)
    keys.forEach(key => {
      storedAlarms.delete(key)
      alarmStatusCache.delete(key)
    })
    console.log(`[内存优化-导出] 清理了 ${entriesToDelete} 个过期告警，当前大小: ${storedAlarms.size}`)
  }
}
...
// ✅ 内存优化：限制latestSamples的大小
const MAX_SAMPLE_AGE = 300000 // 5分钟（从600000ms降低）
const SAMPLE_CLEANUP_INTERVAL = 60000 // 每1分钟检查一次
let lastSampleCleanup = Date.now()

// 清理过期的样本数据
function cleanupLatestSamples() {
  const now = Date.now()
  if (now - lastSampleCleanup < SAMPLE_CLEANUP_INTERVAL) {
    return
  }
  
  lastSampleCleanup = now
  const clients = getModbusClients()
  const activeIps = new Set(Object.keys(clients).filter(ip => {
    const client = clients[ip]
    return client && client.isSetDataExport && !client.isStopped
  }))
  ...
  Object.keys(latestSamples).forEach(api => {
    Object.keys(latestSamples[api]).forEach(ip => {
      if (!activeIps.has(ip)) {
        delete latestSamples[api][ip]
        if (lastWritten[api]) {
          delete lastWritten[api][ip]
        }
        cleanedCount++
      }
    })
  ...
  if (cleanedCount > 0) {
    console.log(`[内存优化-导出] 清理了 ${cleanedCount} 个非活跃IP的样本数据`)
  }
}
...
// ✅ 内存优化：限制CSV缓冲区大小
const MAX_BUFFER_LINES_PER_FILE = 2000 // 每个文件最多缓存2000行（从5000降低）
const BUFFER_WARN_THRESHOLD = 1500 // 超过1500行时发出警告
  ...
  csvMutex
    ...
    .then((release) => {
      try {
        ...
        // ✅ 内存优化：限制缓冲区大小，防止内存爆炸
        if (entry.buffer.length >= MAX_BUFFER_LINES_PER_FILE) {
          console.warn(`[内存优化-导出] CSV缓冲区已满(${entry.buffer.length}行)，丢弃旧数据: ${filePath}`)
          // 删除最旧的一半数据，保留最新的一半
          entry.buffer.splice(0, Math.floor(entry.buffer.length / 2))
        } else if (entry.buffer.length >= BUFFER_WARN_THRESHOLD) {
          console.warn(`[内存警告] CSV缓冲区接近上限(${entry.buffer.length}/${MAX_BUFFER_LINES_PER_FILE}行): ${filePath}`)
        }
