;('use strict')
// ✅ 内存优化：限制缓存大小，超过限制时清理最旧的数据
const MAX_CACHE_SIZE = 20 // 最多缓存20个IP的数据（容量翻倍以适应更多设备）
...
function limitCacheSize(cache, maxSize = MAX_CACHE_SIZE) {
  if (cache.size > maxSize) {
    ...
    console.log(`[内存优化] 清理了 ${entriesToDelete} 个缓存条目，当前大小: ${cache.size}`)
  }
}
...
const readModbusData_cellData = async function (mtclient) {
  ...
  // ✅ 内存优化：限制cellDataCache大小
  limitCacheSize(cellDataCache)
...
const readModbusData_packClusterData = async function (mtclient) {
  ...
  try {
    ...
    // ✅ 内存优化：限制packDataCache和clusterDataCache大小
    limitCacheSize(packDataCache)
    limitCacheSize(clusterDataCache)
...
// ✅ 内存优化：限制告警时间戳缓存大小
const MAX_ALARM_CACHE_SIZE = 5000 // 最多缓存5000个告警时间戳（从10000降低）
const ALARM_CACHE_CLEANUP_INTERVAL = 300000 // 每5分钟清理一次（从600000ms降低）
let lastAlarmCacheCleanup = Date.now()

// 清理过期的告警时间戳缓存
function cleanupAlarmCache() {
  const now = Date.now()
  if (now - lastAlarmCacheCleanup < ALARM_CACHE_CLEANUP_INTERVAL) {
    return // 还没到清理时间
  }
  
  lastAlarmCacheCleanup = now
  
  if (alarmFirstSeenTime.size > MAX_ALARM_CACHE_SIZE) {
    // 清理最旧的一半条目
    const entriesToDelete = Math.floor(alarmFirstSeenTime.size / 2)
    const keys = Array.from(alarmFirstSeenTime.keys()).slice(0, entriesToDelete)
    keys.forEach(key => {
      alarmFirstSeenTime.delete(key)
      previousAlarms.delete(key) // 同时清理previousAlarms
    })
    console.log(`[内存优化] 清理了 ${entriesToDelete} 个过期告警时间戳，当前大小: ${alarmFirstSeenTime.size}`)
  }
}
