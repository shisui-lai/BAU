...
const scales = ref([8, 9, 10, 11, 12, 13, 14, 15, 16])
...
const { setScale, layoutConfig, saveConfig } = useLayout()

const onConfigButtonClick = () => {
  visible.value = !visible.value
}
const onChangeTheme = (theme, mode) => {
  // 手动切换主题以确保路径正确（兼容打包后的环境）
  const themeLink = document.getElementById('theme-css')
  if (themeLink) {
    // 保存当前的 basePath
    const currentHref = themeLink.getAttribute('href')
    const basePath = currentHref.substring(0, currentHref.indexOf('themes/'))
    
    // 创建新的主题路径
    const newHref = `${basePath}themes/${theme}/theme.css`
...
const BTN_W = 48
const BTN_H = 48
const pos = ref({ x: window.innerWidth - 48, y: window.innerHeight / 2 }) // 初始位置
...
function onMouseMove(e) {
  ...
  // 限制到 [min, max]
  const maxX = window.innerWidth - BTN_W
  const maxY = window.innerHeight - BTN_H

  pos.value.x = Math.min(Math.max(0, x), maxX)
  pos.value.y = Math.min(Math.max(0, y), maxY)
}
function onMouseUp() {
  dragging.value = false
}
onBeforeMount(() => {
  // 应用字体大小（确保正确应用，即使 main.js 已经设置过）
  applyScale()
  
  // 同步 inputStyle 到 PrimeVue
  if (layoutConfig.inputStyle.value) {
    $primevue.config.inputStyle = layoutConfig.inputStyle.value
  }
  
  // 同步 ripple 到 PrimeVue
  if (layoutConfig.ripple.value !== undefined) {
...
<template>
  <div v-if="props.inline" class="app-config-inline">
    <div class="p-2">
      <section
        class="pb-4 flex align-items-center justify-content-between border-bottom-1 surface-border"
      >
        <span class="text-xl font-semibold mr-2">Scale</span>
        <div
  ...
  <div v-else>
    ...
    <Sidebar
      ...
      position="right"
      class="layout-config-sidebar w-28rem"
