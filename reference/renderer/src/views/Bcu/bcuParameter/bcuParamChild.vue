...
function getDropdownOptions(el) {
  return getTranslatedOptions(getDropdownKey(el.label), t)
}
...
function getInputValue(el) {
  ...
  if (isDropdownField(el)) {
    ...
    // 特殊处理balancingThresholdK字段
    if (getDropdownKey(el.label) === 'balancingThresholdK') {
      const numValue = Number(value)
      // 如果值不是预设值(10,100,1000)，显示为20mV选项
      if (![10, 100, 1000].includes(numValue)) {
        return 20
      }
      return numValue
    }
