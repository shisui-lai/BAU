...
import { initCellV, initCellT, initCellSOC, initCellSOH, initPackData } from './initData.js'
...
const cellVltg = ref(initCellV)
const cellTemp = ref(initCellT)
const cellSOC = ref(initCellSOC)
const cellSOH = ref(initCellSOH)
const DataPackSumm1 = ref(initPackData)
...
watch(
  ...
  (newIp) => {
    ...
    if (state.deviceData[newIp]) {
      ...
      // 如果该IP没有数据，重置为初始值
      cellVltg.value = initCellV
      cellTemp.value = initCellT
      cellSOC.value = initCellSOC
      cellSOH.value = initCellSOH
      DataPackSumm1.value = initPackData
    }
  }
)
  ...
  (newStatus, oldStatus) => {
    Object.keys(newStatus).forEach((ip) => {
      ...
      if (
        ...
        console.log(`设备 ${ip} 断开连接，清除单体数据缓存`)
        ...
        const events = [
          ...
          'update-FC04SOC',
          'update-FC04SOH',
          'update-FC04dataPackSumm1'
        ]
        events.forEach((event) => {
          localStorage.removeItem(`${event}-${ip}`)
        })
...
onBeforeUnmount(() => {
  listenerIds.value.forEach(({ eventName, handler }) => {
    window.electron.ipcRenderer.removeAllListeners(eventName, handler)
  })
  listenerIds.value = []
})
// 当前视图配置
  ...
  <div>
    ...
    <div>
      ...
      <div class="data-group">
        <div class="data-container"><clusterData :key="viewStore.activeView" /></div>
          ...
          >
            <template #body="{ data }">
              <div class="voltage-cell" :class="getBalanceClass(data.packID, data.afeIdx, col - 1)">
                <template
                  v-if="data.cells[col - 1]?.value !== null && data.cells[col - 1].value !== ''"
                >
      ...
      <!-- BMU数据显示 -->
      <template v-else>
        <DataTable :value="DataPackSumm1" showGridlines style="margin-top: 1rem; width: 100%">
          <Column>
              ...
              >
                <div v-for="item in slotProps.data.element" :key="item.id">
                  <div class="value">
                    ...
                    <div v-if="item.value !== null && item.value !== ''" class="value-content">
                      {{ item.value }}
                      <span
                        v-if="slotProps.data.classification === 'ImpetusTemp' && item.index"
                        class="index-number"
                      >
                        #{{ item.index }}
                      </span>
...
.value-content {
  margin-top: 0.5rem; /* 给 value 留出空间，避免和 label 重叠 */
