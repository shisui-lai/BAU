        ...
        <div class="flex gap-8 align-items-center mt-2">
          <div class="flex align-items-center gap-3">
            ...
            <Button
              ...
              @click="confirmExport"
              :disabled="!canExport || exportStore.isExporting"
...
const startExport = () => {
  ...
  if (storedCount.value === 0) {
    toast.add({ severity: 'warn', summary: t('eventTime.toast.enterExportZero'), life: 3000 })
    return
  }
  stopReadingAll('all')
  stopReading()
...
function confirmExport() {
  ...
  if (!canExport.value) {
    ...
    if (storedCount.value === 0) {
      toast.add({ severity: 'warn', summary: t('eventTime.toast.enterExportZero'), life: 3000 })
    } else if (!flagDownloadAll.value && (!valueRead.value || parseInt(valueRead.value, 10) <= 0)) {
