...
const hasActiveAdaptation = computed(() => {
  return Object.keys(multiBCUStatus.value).length > 0 && 
    Object.values(multiBCUStatus.value).some(status => 
...
const hasActiveBCUAdaptation = computed(() => {
  return Object.keys(bcuAdaptStatus.value).length > 0 && 
    Object.values(bcuAdaptStatus.value).some(status => 
...
const hasAnyActiveAdaptation = computed(() => {
  return hasActiveBCUAdaptation.value || hasActiveAdaptation.value
})
...
const statusSummary = computed(() => {
  ...
  const pending = statuses.filter(s => s.status && !['BMU自适应地址成功', 'BMU自适应地址失败'].includes(s.status)).length
...
const getStatusIcon = (status) => {
  ...
  if (status && !['BMU自适应地址成功', 'BMU自适应地址失败'].includes(status)) return 'pi pi-spin pi-spinner'
  return 'pi pi-clock'
}
...
const sendBMUValue = async () => {
  // 检查所选 IP 是否都在通讯状态
  const notCommunicatingIps = selectedBCUs.value.filter(ip => 
    !ipStore.getIpCommunicationActive(ip)
  )
  
  if (notCommunicatingIps.length > 0) {
    const ipLabels = notCommunicatingIps.map(ip => {
  ...
  window.electron.ipcRenderer.on('address-adapt-result', (event, result) => {
    ...
    if (result.success) {
      toast.add({
        severity: 'success',
        summary: '自适应成功',
        detail: `成功分配 ${result.assigned} 台设备，请重新查询在线ip`,
        life: 5000
      })
 ...
 /*  window.electron.ipcRenderer.removeAllListeners(channel) */ // 移除设备消息监听
  listenerId.value = (event, Arg) => {
    ...
    if (status === 'BMU自适应地址成功' || status === 'BMU自适应地址失败') {
      ...
      // 检查是否所有BCU都完成了自适应
      if (pendingAdaptIps.size === 0 && Object.keys(multiBCUStatus.value).length > 0 && !hasShownCompletionToast.value) {
        const successCount = Object.values(multiBCUStatus.value).filter(s => s.status === 'BMU自适应地址成功').length
        const failedCount = Object.values(multiBCUStatus.value).filter(s => s.status === 'BMU自适应地址失败').length
        ...
        <div class="adaptation-table-container">
          ...
          >
            ...
            <Column field="error" :header="t('addressAdapt.bcu.tableHeaders.error')">
              <template #body="slotProps">
                <div v-if="slotProps.data.error && slotProps.data.status === 'BCU地址自适应失败'" class="failure-reason-cell">
                  <span>{{ 
                    locale === 'zh' 
            ...
            <Column field="timestamp" :header="t('addressAdapt.bcu.tableHeaders.timestamp')">
              <template #body="slotProps">
                <span v-if="slotProps.data.timestamp" class="timestamp">{{ slotProps.data.timestamp }}</span>
      ...
      <div class="card">
        ...
        <div class="adaptation-table-container">
          ...
          >
            ...
            <Column field="failureReason" :header="t('addressAdapt.bmu.tableHeaders.failureReason')">
              <template #body="slotProps">
                <div v-if="slotProps.data.failureReason && slotProps.data.status === 'BMU自适应地址失败'" class="failure-reason-cell">
                  <span>{{ 
                    locale === 'zh' 
