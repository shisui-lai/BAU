...
let autoStopTimer = null // 添加定时器引用，用于取消
function handleDiskSpaceWarning() {
  ...
  confirm.require({
    message: t('export.diskWarning.confirmMessage'),
    header: t('export.diskWarning.confirmHeader'),
    ...
    accept: async () => {
      ...
      toast.add({
        severity: 'warn',
        summary: t('export.diskWarning.manualStopSummary'),
        detail: t('export.diskWarning.manualStopDetail'),
        life: 5000
      })
    },
    reject: () => {
  ...
  autoStopTimer = setTimeout(async () => {
    if (diskWarningActive.value) {
      ...
      toast.add({
        severity: 'warn',
        summary: t('export.diskWarning.autoStopSummary'),
        detail: t('export.diskWarning.autoStopDetail'),
        life: 5000
      })
    }
  }, 10000)
