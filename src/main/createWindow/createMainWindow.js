...
function createMainWindow() {
  ...
  mainWindow.on('close', (event) => {
    ...
    if (result === 0) {
      ...
      // 通知 renderer 进程停止所有均衡操作
      mainWindow.webContents.send('app-closing-request')

      // 设置超时，如果 10 秒内没有响应，强制退出
      mainWindow.quitTimeout = setTimeout(() => {
        console.warn('均衡停止超时，强制退出应用')
        forceQuit()
      }, 10000)
    }
  })
...
function forceQuit() {
  // 清除超时定时器
  if (mainWindow.quitTimeout) {
    clearTimeout(mainWindow.quitTimeout)
  }
...
export { mainWindow, mainWindowActive, createMainWindow, forceQuit }
