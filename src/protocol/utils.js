...
export function parseByTable(view, table, start = 0) {
  ...
  for (const fld of table) {
    ...
    if (type === 'bits') {
      ...
      base[key] = {
        raw, // 0-3
        txt: fld.map ? (fld.map[raw] ?? String(raw)) : String(raw) // 若无 map → 返回数值字符串
      }
      continue
    }
    ...
    /* ---------- 专用两字节BOOT字符串（小端） ------------------------- */
    if (type === 'boot_str2_le') {
      const b0 = view.getUint8(off)
      const b1 = view.getUint8(off + 1)
      off += 2
      base[key] = ascii.decode(new Uint8Array([b1, b0])).replace(/\0/g, '') || '--'
      continue
    }
    /* ---------- 固定长 ASCII 字符串 --------------------------------- */
    ...
    if (off + typeSize > view.byteLength) {
      throw new RangeError(
        `Offset ${off} is outside the bounds of the DataView (length ${view.byteLength}) for field ${key} (${type})`
      )
    }
    ...
    if (type === 'hex' || type === 'hex16') {
      ...
      if (fld.map && typeof fld.map === 'object') {
        ...
        base[key] = {
          raw: rawVal,
          txt:
            fld.map[rawVal] !== undefined
              ? fld.map[rawVal]
              : `未知值(0x${rawVal.toString(16).toUpperCase()})`
        }
      } else {
...
export function parseByTableWithSkip(view, table, start = 0) {
  ...
  for (const fld of table) {
    ...
    try {
      switch (type) {
        case 'u8':
          rawValue = view.getUint8(off, true)
          off += 1
          break
        case 's8':
          rawValue = view.getInt8(off, true)
          off += 1
          break
        case 'u16':
          ...
          break
        case 's16':
          rawValue = view.getInt16(off, true)
          off += 2
          break
        case 'u32':
          rawValue = view.getUint32(off, true)
          off += 4
          break
        case 's32':
          rawValue = view.getInt32(off, true)
          off += 4
          break
        case 'f32':
          rawValue = view.getFloat32(off, true)
          off += 4
          break
        case 'hex':
          rawValue = view.getUint16(off, true)
        ...
        case 'str2':
          ...
          continue
        case 'boot_str2_le': {
          const b0 = view.getUint8(off, true)
          const b1 = view.getUint8(off + 1, true)
          off += 2
          if (shouldStore) {
            const decoded = ascii.decode(new Uint8Array([b1, b0])).replace(/\0/g, '')
            base[key] = decoded || '--'
          }
          continue
        }
...
export function groupByClass(schema, flat) {
  ...
  for (const field of schema) {
    ...
    const value = field.map && field.map[rawValue] !== undefined ? field.map[rawValue] : rawValue

    sections[cls].push({ key: field.key, label: field.label, value })
  }
...
export function serialize(schema, dataObj, offset, length) {
  ...
  for (const f of schema) {
    ...
    /* 普通 16 位 */
    put16((dataObj[key] ?? 0) * scale, type.startsWith('s'))
    inc()
  }
...
export function processHardwareFaultRAW(hex) {
  ...
  // ① 读 DataLength
  const { baseConfig: head, nextOffset: o1 } = parseByTable(
    view,
    [{ key: 'DataLength', type: 'u16' }],
    0
  )
...
export function processBrokenwireRAW(hexString) {
  ...
  const { baseConfig: head, nextOffset: offHdr } = parseByTable(view, CELL_HEADER, 0)
...
export function parseWriteResponse(payload) {
  ...
  if (buf.length === 1) {
    ...
    console.log(
      `write response: ${statusCode.toString(16)} (${ERROR_CODES[statusCode] || '未知状态'})`
    )
...
export function parseQueryResponse(payload, commandTopic = 'unknown') {
  try {
    ...
    if (buf.length === 1) {
      ...
      console.log(
        `[查询应答] 错误码: 0x${statusCode.toString(16).toUpperCase()} (${ERROR_CODES[statusCode] || '未知状态'})`
      )
    ...
    if (buf.length === 20) {
      ...
      return {
        ...
        data: {
          ...
          downloadCompleteFlagRaw:
            typeof downloadCompleteFlag === 'object'
              ? downloadCompleteFlag.raw
              : downloadCompleteFlag,
          completionTypeRaw:
            typeof completionType === 'object' ? completionType.raw : completionType,
          ...
          bcuFaultCode:
            typeof bcuFaultCode === 'object'
              ? bcuFaultCode.txt
              : bcuFaultCode === 0
                ? '无故障'
                : `故障码: 0x${bcuFaultCode.toString(16).toUpperCase()}`,
          bmuFaultCode:
            typeof bmuFaultCode === 'object'
              ? bmuFaultCode.txt
              : bmuFaultCode === 0
                ? '无故障'
                : `故障码: 0x${bmuFaultCode.toString(16).toUpperCase()}`,
          bmuFailedDevices,
          totalPackets,
          currentPacket,
          success: true
    ...
    if (buf.length === 6) {
      ...
      return {
        ...
        data: {
          ...
          otaErrorCode: typeof otaErrorCode === 'object' ? otaErrorCode.txt : otaErrorCode,
          bauFaultCode:
            typeof bauFaultCode === 'object'
              ? bauFaultCode.txt
              : bauFaultCode === 0
                ? '无故障'
                : `故障码: 0x${bauFaultCode.toString(16).toUpperCase()}`,
          success: true
        }
      }
    }
...
export function parseBlockCommonParamRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseBlockCommonParamRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function parseBlockTimeCfgRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseBlockTimeCfgRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
  const view = new DataView(paramsBuf.buffer, paramsBuf.byteOffset, paramsBuf.byteLength)
...
export function parseSysRunTimeRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseSysRunTimeRAW] length mismatch: dataLen field=${dataLen}, actual buffer length=${paramsBuf.length}`
    )
  }
...
function processSysRunTimeData(baseConfig, schema) {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    // 运行时间（分钟）
    if (
      processedConfig[`${prefix}RunTime_Low`] !== undefined &&
      processedConfig[`${prefix}RunTime_High`] !== undefined
    ) {
      const runTime =
        processedConfig[`${prefix}RunTime_Low`] | (processedConfig[`${prefix}RunTime_High`] << 16)
    ...
    // 周期任务堆栈大小（字节）
    if (
      processedConfig[`${prefix}PeriodicStack_Low`] !== undefined &&
      processedConfig[`${prefix}PeriodicStack_High`] !== undefined
    ) {
      const stackSize =
        processedConfig[`${prefix}PeriodicStack_Low`] |
        (processedConfig[`${prefix}PeriodicStack_High`] << 16)
    ...
    // 系统堆栈空间（字节）
    if (
      processedConfig[`${prefix}SystemStack_Low`] !== undefined &&
      processedConfig[`${prefix}SystemStack_High`] !== undefined
    ) {
      const stackSize =
        processedConfig[`${prefix}SystemStack_Low`] |
        (processedConfig[`${prefix}SystemStack_High`] << 16)
    ...
    // 系统堆栈最小空间（字节）
    if (
      processedConfig[`${prefix}SystemStackMin_Low`] !== undefined &&
      processedConfig[`${prefix}SystemStackMin_High`] !== undefined
    ) {
      const stackSize =
        processedConfig[`${prefix}SystemStackMin_Low`] |
        (processedConfig[`${prefix}SystemStackMin_High`] << 16)
...
export function parseBlockPortCfgRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseBlockPortCfgRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
  const view = new DataView(paramsBuf.buffer, paramsBuf.byteOffset, paramsBuf.byteLength)
...
export function parseClusterDnsParamRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `cluster_dns_param_r length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function parseRealTimeSaveRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseRealTimeSaveRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
  const view = new DataView(paramsBuf.buffer, paramsBuf.byteOffset, paramsBuf.byteLength)
...
export function parseSOXCfgParamRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseSOXCfgParamRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
  const view = new DataView(paramsBuf.buffer, paramsBuf.byteOffset, paramsBuf.byteLength)
...
export function parseSOCCfgParamRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseSOCCfgParamRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
  const view = new DataView(paramsBuf.buffer, paramsBuf.byteOffset, paramsBuf.byteLength)
...
export function parseSOHCfgParamRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseSOHCfgParamRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
  const view = new DataView(paramsBuf.buffer, paramsBuf.byteOffset, paramsBuf.byteLength)
...
export function parseCluAnalogFaultLevelSumRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `clu_analog_fault_level_sum length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
  ...
  if (dataBuf.length < expectedDataLength) {
    console.error(
      `[ERROR] 数据长度不足: 需要 ${expectedDataLength} 字节，实际只有 ${dataBuf.length} 字节`
    )
...
export function parseBlockAnalogFaultLevelRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `block_analog_fault_level length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function parseBlockAnalogFaultGradeRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `block_analog_fault_grade length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function parseCluAnalogFaultGradeRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `clu_analog_fault_grade length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function parseBlockBattParamRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseBlockBattParamRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function parseBlockCommDevCfgRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseBlockCommDevCfgRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function parseBlockOperateCfgRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseBlockOperateCfgRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function parseBlockSocParamRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseBlockSocParamRAW] length mismatch: expected ${dataLen}, got ${paramsBuf.length}`
    )
  }
...
export function processBcuAdaptiveQueryResult(hex) {
  ...
  if (buf.length === 8) {
    ...
    console.log(
      `[BCU地址自适应查询] 解析成功: 寄存器[0x${register1.toString(16)}, 0x${register2.toString(16)}, 0x${register3.toString(16)}, 0x${register4.toString(16)}]`
    )
  ...
  // 其他长度认为是异常
  console.warn(
    `[BCU地址自适应查询] 意外的响应长度: ${buf.length}字节，期望1字节(错误)或8字节(成功)`
  )
  return {
    error: true,
    data: {
...
export function processBmuAdaptiveQueryResult(hex) {
  ...
  if (buf.length === 10) {
    ...
    console.log(
      `[BMU地址自适应查询] 解析成功: 簇${currentCluster}, 状态0x${status.toString(16)}, 内容[0x${content1.toString(16)}, 0x${content2.toString(16)}, 0x${content3.toString(16)}]`
    )
  ...
  if (buf.length > 10 && buf.length % 10 === 0) {
    ...
    for (let i = 0; i < clusterCount; i++) {
      ...
      console.log(
        `[BMU地址自适应查询] 簇${currentCluster}: 状态0x${status.toString(16)}, 内容[0x${content1.toString(16)}, 0x${content2.toString(16)}, 0x${content3.toString(16)}]`
      )
    }
  ...
  // 其他长度认为是异常
  console.warn(
    `[BMU地址自适应查询] 意外的响应长度: ${buf.length}字节，期望1字节(错误)或10字节(单簇)或10*N字节(多簇)`
  )
  return {
    error: true,
    data: {
...
function processCellDataRAW(hex, resolution, tag = 'cell', isSigned = false) {
  ...
  const valueArr = []
  for (let i = 0; i < count; i++) {
    const raw = isSigned ? pick.s16(view, nextOffset + i * 2) : pick.u16(view, nextOffset + i * 2)
...
export function parseEventRecordFlagRAW(payload) {
  ...
  if (paramsBuf.length !== dataLen) {
    console.warn(
      `[parseEventRecordFlagRAW] 数据长度不匹配: 期望 ${dataLen} 字节，实际 ${paramsBuf.length} 字节`
    )
  }
...
export function parseEventRecordRAW(payload) {
  ...
  if (dataLen !== expectedDataLength && recordCount > 0) {
    console.warn(
      `[parseEventRecordRAW] 数据长度不匹配: header中为${dataLen}字节，计算为${expectedDataLength}字节（记录数${recordCount}）`
    )
  }
  ...
  if (remainingData < RECORD_TOTAL_SIZE && recordCount === 0) {
    console.warn(
      `[parseEventRecordRAW] 数据长度不足: 需要至少${RECORD_TOTAL_SIZE}字节（1条记录），实际 ${remainingData} 字节`
    )
  }
  ...
  for (let i = 0; i < recordCount; i++) {
    ...
    if (offset + RECORD_TOTAL_SIZE > buf.length) {
      console.warn(
        `[parseEventRecordRAW] 记录${i}数据不足: 需要${RECORD_TOTAL_SIZE}字节，剩余 ${buf.length - offset} 字节`
      )
      break
    }
    ...
    if (recordDataBuf.length < RECORD_DATA_SIZE) {
      console.warn(
        `[parseEventRecordRAW] 记录${i}数据长度不足: 需要${RECORD_DATA_SIZE}字节，实际 ${recordDataBuf.length} 字节`
      )
      break
    }
    ...
    if (
      baseConfig.Year !== undefined &&
      baseConfig.Month !== undefined &&
      baseConfig.Day !== undefined
    ) {
      // BCD解码函数
