...
export function useSOXParam() {
  ...
  const createDefaultSOXParamData = () =>
    createDefaultParameterData(ALL_SOX_PARAM_R, '[useSOXParam]')

  const parseSOXParamReadResponse = (mqttMessage) =>
    parseParameterReadResponse(mqttMessage, '[useSOXParam]', 'SOX参数')

  const parseSOXParamWriteResponse = (mqttMessage) =>
    parseParameterWriteResponse(mqttMessage, '[useSOXParam]', 'SOX参数')
