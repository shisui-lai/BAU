...
export const useBlockStore = defineStore('block', () => {
  ...
  const showBlockSelector = computed(() => {
    return currentPageType.value === 'block' || currentPageType.value === 'blockread'
  })
  ...
  function replaceBlockOptions(newOptions) {
    ...
    const validOptions = newOptions.filter((option) => {
      const isValid = option && typeof option.value === 'string' && isValidBlockKey(option.value)
    ...
    if (!isSame) {
      ...
      console.log(
        '[blockStore] replaceBlockOptions: replaced with',
