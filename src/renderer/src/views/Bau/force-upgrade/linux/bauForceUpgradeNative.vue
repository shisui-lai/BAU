    ...
    <div class="control">
      ...
      <div class="section1">
        <div class="section-header">
          ...
          <button
            class="btn btn-sm"
            :class="tftpRunning ? 'btn-danger' : 'btn-success'"
            @click="toggleTftp"
            :disabled="isUpgrading"
          >
            <i :class="tftpRunning ? 'pi pi-stop' : 'pi pi-play'"></i>
            <span>{{ tftpRunning ? t('forceUpgrade.stopTftp', '停止TFTP') : t('forceUpgrade.startTftp', '启动TFTP') }}</span>
          </button>
        </div>
        ...
        <div class="input-row">
          ...
          <button
            ...
            :disabled="tftpRunning || isUpgrading"
            :title="t('forceUpgrade.selectFile', '选择文件')"
          >
        ...
        <div class="action-bar">
          <button
            class="btn btn-sm"
            :class="isUpgrading ? 'btn-danger' : 'btn-success'"
            @click="toggleForceUpgrade"
            :disabled="!canStartUpgrade"
          >
            <i :class="isUpgrading ? 'pi pi-stop' : 'pi pi-send'"></i>
            <span>{{ isUpgrading ? t('forceUpgrade.stopUpgrade', '停止升级') : t('forceUpgrade.startUpgrade', '开始升级') }}</span>
          </button>
        </div>
        ...
        <div v-if="isUpgrading" class="sending-status">
          <div class="native-progress-bar native-progress-bar-indeterminate" style="height: 0.5rem">
            <div class="native-progress-bar-fill"></div>
          </div>
      ...
      <div class="section">
        <div class="section-header">
          ...
          >
            ...
            <span>{{ t('forceUpgrade.log.clear', '清空') }}</span>
          </button>
        </div>
        <div class="log-panel">
...
// 已移除 PrimeVue 组件导入（Button, InputText, ProgressBar），使用原生 HTML 元素
...
.native-input:focus {
  ...
  box-shadow: 0 0 0 2px var(--surface-card), 0 0 0 4px rgba(var(--primary-color-rgb), 0.2);
}
...
/* 不确定进度条动画 */
.native-progress-bar-indeterminate {
  overflow: hidden;
}

.native-progress-bar-indeterminate .native-progress-bar-fill {
  width: 30%;
