<template>
  <div class="card">
    <div class="block-info">
      <!-- 原生HTML标签页 -->
      <div class="block-info-tabs">
        <!-- 标签页导航 -->
        <div class="tab-nav">
          <button
            @click="activeTabIndex = 0"
            :class="['tab-button', activeTabIndex === 0 ? 'active' : '']"
          >
            {{ t('config.blockInfoPage.sections.summaryInfo') }}
          </button>
          <button
            @click="activeTabIndex = 1"
            :class="['tab-button', activeTabIndex === 1 ? 'active' : '']"
          >
            {{ t('config.blockInfoPage.sections.systemAbstract') }}
          </button>
        </div>
