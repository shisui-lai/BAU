      ...
      <div v-else>
        ...
        <div class="fault-tabs">
          ...
          <div class="tab-nav">
            ...
            >
              {{ t('faultOverview.blockFaultOverview') }}
            </button>
            <button
              v-for="(cluster, index) in clusterData"
              :key="cluster.id"
              @click="activeTabIndex = index + 1"
              :class="['tab-button', activeTabIndex === index + 1 ? 'active' : '']"
            >
              {{ t('faultOverview.clusterFaultOverview', [cluster.id]) }}
            </button>
          </div>
