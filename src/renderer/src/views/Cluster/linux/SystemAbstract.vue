...
// 内部处理函数（无限流）
function processMessageInternal(_e, msg) {
...
const throttledHandler = throttle(
  (event, message) => {
    processMessageInternal(event, message)
  },
  THROTTLE_INTERVAL,
  {
...
// 清理节流函数
let throttledHandlerCleanup = null

onMounted(() => {
  // 先清理可能存在的旧监听器（防止快速切换导致的残留）
  window.electron.ipcRenderer.removeAllListeners('SYS_ABSTRACT')

  window.electron.ipcRenderer.on('SYS_ABSTRACT', throttledHandler)
  
  // 保存清理函数引用
  throttledHandlerCleanup = () => {
...
onBeforeUnmount(() => {
  window.electron.ipcRenderer.removeAllListeners('SYS_ABSTRACT', throttledHandler)
  
  // 清理节流函数
  if (throttledHandlerCleanup) {
    throttledHandlerCleanup()
  }
  
  // 刷新所有待处理的更新
  if (throttledHandler.flush) {
