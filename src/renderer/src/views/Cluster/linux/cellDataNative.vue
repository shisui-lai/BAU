...
const THROTTLE_CONFIG = {
  ...
  // 簇端数据处理函数（无限流）
  function processClusterSummaryInternal(_e, msg) {
    parseClusterSummary(msg)
  }

  // 簇端数据节流处理函数（限流入口）
  const throttledClusterSummaryHandler = throttle(
    (event, message) => {
      processClusterSummaryInternal(event, message)
    },
    THROTTLE_CONFIG.SUMMARY_DATA,
    {
  ...
  function onClusterSummary (_e, msg) {
    throttledClusterSummaryHandler(_e, msg)
  }
...
function onCellMsg (_e, msg) { 
  ...
  if (msg.dataType === 'CLUSTER_SUMMARY')    { throttledClusterSummaryHandler(_e, msg); return }
}  // 保留原 handler
...
onBeforeUnmount(() => {
  ...
  // 清理簇端数据节流函数
  if (throttledClusterSummaryHandler.cancel) {
    throttledClusterSummaryHandler.cancel()
  }
  if (throttledClusterSummaryHandler.flush) {
    throttledClusterSummaryHandler.flush()
  }
...
<template>
  <div class="card flex flex-col h-full p-3 gap-1">
    ...
    <div class="flex gap-1">
      <button
        v-for="(cfg, key) in DATA_TYPE_MAP"
        :key="key"
        @click="activeView = key"
        :class="[
          'btn',
          'btn-data-type',
          activeView === key ? 'btn-primary' : 'btn-outlined'
        ]"
      >
        {{ cfg.label }}
      </button>
    ...
    <!-- ▼▼ 单体数据表（完整渲染，无内部滚动）▼▼ -->
    <div v-if="['CELL_VOLT','CELL_TEMP','CELL_SOC','CELL_SOH'].includes(activeView)"
         class="table-container-native">
      <table class="native-table" ref="dataTableRef">
        <thead>
          <tr>
            <th class="frozen-column" style="width:80px">{{ t('batteryInfo.table.bmuAfe') }}</th>
            <th v-for="col in displayCols" :key="col" style="width:60px">{{ col }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(row, rowIndex) in matrixRows" :key="rowIndex">
            <td class="frozen-column">{{ row.bmuLabel }}</td>
            <td v-for="col in displayCols" :key="col">
              <span class="cell-content">
                {{ formatCell(row.cells[col-1]) }}<template v-if="row.cells[col-1] !== '--'"> #{{ calculateGlobalCellIndex(row, col) }}</template>
              </span>
            </td>
          </tr>
          <tr v-if="!matrixRows || matrixRows.length === 0">
            <td :colspan="displayCols.length + 1" class="empty-message">
              {{ '无数据' }}
            </td>
          </tr>
        </tbody>
  ...
  .native-table thead .frozen-column {
    background-color: var(--surface-section);
    z-index: 3;
  }

  .native-table tbody tr:hover .frozen-column {
    background-color: var(--surface-hover);
  }
