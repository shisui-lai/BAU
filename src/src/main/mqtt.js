...
function withResponseCheck(fn) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        client.on('connect', () => {
          ...
          // 通知主进程连接成功
          safeSend({ type: 'mqtt-connected', data: { clientId: config.clientId, host: config.host, port: config.port } })
          resolve(true)
        })
        ...
        client.on('close', () => {
          if (isConnected) {
            ...
            safeSend({ type: 'mqtt-disconnected', data: {} })
          }
        })
        ...
        client.on('offline', () => {
          ...
          // 【数据速率】停止速率计算
          stopDataRateCalculation()
          safeSend({ type: 'mqtt-offline', data: {} })
        })
        ...
        client.on('reconnect', () => {
          ...
          // 重连时重置健康检查数据，准备新的连接
          resetHealthCheckData()
          safeSend({ type: 'mqtt-reconnecting', data: {} })
        })  
  ...
  function disconnectMqtt() {
    return new Promise((resolve) => {
      ...
      // 【修复】先移除消息监听器，防止断开过程中收到消息并尝试发送
      if (client && messageHandlerRef) {
        try {
          client.removeListener('message', messageHandlerRef)
          console.log('[MQTT Child] 已移除消息监听器（断开连接）')
        } catch (e) {
          console.warn('[MQTT Child] 移除消息监听器失败:', e.message)
        }
      }
  ...
  let isCleaningUp = false  // 【修复】清理标志，防止清理过程中处理新消息
  ...
  function safeSend(message) {
    ...
    try {
      ...
      if (error.code === 'ERR_IPC_CHANNEL_CLOSED') {
        console.log('[MQTT Child] IPC通道已关闭，停止发送消息')
        isCleaningUp = true
      } else {
        console.error('[MQTT Child] 发送消息失败:', error.message)
      }
      return false
    }
  }
  ...
  function setupMessageHandler() {
    ...
    messageHandlerRef = (topic, payload) => {
      // 【修复】如果正在清理，直接返回，避免向已关闭的IPC通道发送消息
      if (isCleaningUp) {
        return
      }
      ...
      // 【修复】使用安全发送函数，防止IPC通道关闭时崩溃
      if (!safeSend({ type: msg.dataType, data: msg })) {
        return
      }
    ...
    process.on('message', (message) => {
      ...
      if (cmd === 'MQTT_CONNECT') {
        ...
        connectMqtt(config).then(success => {
          ...
          safeSend({ type: 'mqtt-connect-result', data: { success } })
        }).catch(error => {
          console.error('[MQTT Child]  连接失败:', error)
          safeSend({ type: 'mqtt-connect-result', data: { success: false, error: error.message } })
        })
        return
      }
      ...
      if (cmd === 'MQTT_DISCONNECT') {
        // 断开MQTT连接
        disconnectMqtt().then(() => {
          safeSend({ type: 'mqtt-disconnect-result', data: { success: true } })
        })
        return
      }
      ...
      if (cmd === 'MQTT_TEST_CONNECTION') {
        ...
        testMqttConnection(config).then(result => {
          safeSend({ type: 'mqtt-test-result', data: result })
        })
        return
      }
  ...
  function cleanUp(){
    // 【修复】设置清理标志，防止消息处理器继续处理新消息
    isCleaningUp = true
    
    // 【修复】先停止接收新消息，再断开连接
    // 顺序很重要：先移除监听器，再断开连接，避免断开过程中收到消息
    if (client && messageHandlerRef) {
      try {
        client.removeListener('message', messageHandlerRef)
        console.log('[MQTT Child] 已移除消息监听器')
      } catch (e) {
        console.warn('[MQTT Child] 移除消息监听器失败:', e.message)
      }
    }
    ...
    if (client) {
      try {
        ...
        client.removeAllListeners();
        console.log('[MQTT Child] 已断开MQTT连接')
      } catch (e) {
        console.warn('[MQTT Child] 断开MQTT连接失败:', e.message)
      }
    }
    ...
    try {
      ...
      console.warn('[MQTT Child] 移除进程消息监听器失败:', e.message)
    }
    
    console.log('[MQTT Child] 清理完成')
  }
