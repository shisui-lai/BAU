...
class modbusTcpClient {
  ...
  async finalizeConnectSuccess() {
    ...
    this.arpCacheCleared = false // 连接成功后重置ARP缓存清除标志位
  ...
  handleConnectError(e) {
    ...
    // 清除ARP缓存策略：重连次数大于2次时执行一次
    if (isAdmin && this.retryCount > 2 && !this.arpCacheCleared) {
      console.log(`[${this.mbsHost}] 重连次数 ${this.retryCount} 次，开始清除ARP缓存`)
      clearArpCache(this.mbsHost)
      this.arpCacheCleared = true
    }
