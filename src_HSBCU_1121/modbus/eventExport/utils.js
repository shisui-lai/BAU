...
function parseSysConfig1(raw, whichConfig) {
  ...
  switch (whichConfig) {
    ...
    case 2: {
      ...
      const disableEnableCluster = (raw >> 15) & 0x01 // 提取 bit15
      const parts = [
        ...
        `禁止使能簇配置:${configsMap.disableEnableCluster[disableEnableCluster] ?? `未知(${raw})`}`
      ]
      return parts.join(';')
    }
    case 3: {
...
function formatDateTimeForEvent(date) {
  const year = date.getFullYear() // 返回完整的年份（4 位）
  const month = (date.getMonth() + 1).toString().padStart(2, '0') // 月份 0–11，需要 +1
  const day = date.getDate().toString().padStart(2, '0') // 当月第几天 1–31
  const hour = date.getHours().toString().padStart(2, '0') // 0–23，补齐两位
  const minute = date.getMinutes().toString().padStart(2, '0') // 0–59，补齐两位
  const second = date.getSeconds().toString().padStart(2, '0') // 0–59，补齐两位

  // 修改为 YYYY-MM-DD-HH:mm:ss 格式，避免Excel自动隐藏秒数
