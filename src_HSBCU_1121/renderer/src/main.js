...
async function loadSavedThemeConfig() {
  try {
    ...
    if (config.theme) {
      ...
      if (themeLink) {
        const currentTheme = themeLink.getAttribute('href').match(/themes\/(.+)\/theme\.css/)?.[1]
        if (currentTheme && currentTheme !== config.theme) {
          // 获取正确的主题路径（相对路径，兼容打包后的环境）
          const currentHref = themeLink.getAttribute('href')
          const basePath = currentHref.substring(0, currentHref.indexOf('themes/'))
          const newHref = `${basePath}themes/${config.theme}/theme.css`
