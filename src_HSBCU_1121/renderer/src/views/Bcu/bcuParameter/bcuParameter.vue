...
async function exportAllModules() {
  ...
  try {
    ...
    // 使用 exportIp 获取当前页面A的数据（实时更新的数据）
    ...
    const [dataBResult, dataCResult] = await Promise.all([
      readOnce('update-FC04ConfigAlarm', MODULE_B, initDataAlarmConfig, mergeData, 'pageAlarm', exportIp)
        .then((d) => {
          // 将读取的数据保存到 store，并返回深拷贝的数据用于导出
          moduleData.setModuleData(MODULE_B, exportIp, d)
          return JSON.parse(JSON.stringify(d))
        })
        .catch(() => moduleData.getModuleData(MODULE_B, exportIp) || initDataAlarmConfig()),
...
const applyImportedData = async (rows) => {
  ...
  const [rawB, rawC] = await Promise.all([
    readOnce('update-FC04ConfigAlarm', MODULE_B, initDataAlarmConfig, mergeData, 'pageAlarm', ipStore.selectedIp)
      .then((d) => {
