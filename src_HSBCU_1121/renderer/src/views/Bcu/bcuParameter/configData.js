...
export async function writeImportedForData(data, ipStore, toast, t) {
  ...
  flatData.forEach((el) => {
    ...
    if (el.dataType === 'ip') {
      ...
      // BCU 三个32位合并寄存器（簇校正电量/额定电量/额定功率）
      // 与页面手动编辑保持一致：按两位小数放大100倍拆分，再以两位小数写回两个16位寄存器
      const paired = mergeRegisters.get(addr)
      const numericValue = parseFloat(el.importedValue)
      if (!isNaN(numericValue)) {
        const scaledValue = Math.round(numericValue * 100)
        const high = (scaledValue >>> 16) & 0xffff
        const low = scaledValue & 0xffff
        writeData.push({ address: addr, value: +(low / 100).toFixed(2) })
        writeData.push({ address: paired, value: +(high / 100).toFixed(2) })
      }
    } else {
      // 普通16位寄存器
