...
async function onExportCsv() {
  ...
  // ——— 只导出实际行列数的充放功率表格数据 ———
  // 1. 充电功率表格 - 使用充电表格的实际行列数
  const chargeRows = data.chargeTable.slice(0, rowNumChrg.value)
  const chargeCols = data.colTitlesChrg.slice(0, colNumChrg.value)
  ...
  // 2. 放电功率表格 - 使用放电表格的实际行列数
  const dischargeRows = data.dischargeTable.slice(0, rowNumDis.value)
  const dischargeCols = data.colTitlesDis.slice(0, colNumDis.value)
