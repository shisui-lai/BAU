  ...
  <div class="card">
    <div style="display: flex; flex-direction: column; gap: 0.5rem; padding: 1rem">
      ...
      <div style="display: flex; gap: 0.8rem; align-items: center">
        ...
        <Button
          @click="queryDevices"
          :disabled="!selectedInterface || isQuerying || ipstore.isAutoQueryingIp"
...
// ✅ 新增：监听 ipstore.ipList 变化，同步更新 modbusClients 和 responses
watch(
  () => ipstore.ipList,
  (newIpList) => {
    //console.log('📡 ipstore.ipList changed, updating modbusClients and responses:', newIpList)

    // 🔒 防止递归更新：只在 modbusClients 的 IP 列表与 newIpList 不同时才更新
    const currentIps = modbusClients.value.map((c) => c.ModbusServerIP)
    const hasChanged =
      newIpList.length !== currentIps.length ||
      newIpList.some((ip, index) => ip !== currentIps[index])
    ...
    // 更新 modbusClients
    modbusClients.value = newIpList.map((ip) => ({
      ModbusServerIP: ip,
      ModbusServerSum: 25,
      skills: []
    }))
    // 同步更新 responses（基于ipstore.responses）
    if (ipstore.responses && ipstore.responses.length > 0) {
      responses.value = ipstore.responses
    }
  },
  { deep: true }
)
