        ...
        <div>
          ...
          >
            ...
            <Button
              ...
              :icon="ftpIcon"
              iconPos="left"
              :disabled="upgradeState === UPGRADE_STATE.SENDING_INSTRUCTIONS"
            />
          </div>

          <div class="input-row">
            {{ ipstore.networkInterfaces }}
          ...
          <div class="input-row" style="display: none">
            ...
            <Button
              ...
              :disabled="ftpRunning || upgradeState === UPGRADE_STATE.SENDING_INSTRUCTIONS"
            />
          </div>
        </div>
          ...
          <Button
            ...
            :icon="
              (someCommunicating = ipstore.ipList.some((ip) => ipstore.getIpCommunicationActive(ip))
                ...
                : 'pi pi-play')
            "
            :disabled="!hasActiveConnection || upgradeState === UPGRADE_STATE.SENDING_INSTRUCTIONS"
          />
          <Button
...
// 存储每个IP是否已完成文件下载，等待BMU重启（用于强制升级0xc0b1和0xc0b2）
const fileDownloadCompleted = reactive({}) // { [ip: string]: boolean }
// BMU Progress State
...
const UPGRADE_TIMEOUT_MS = 300000
// —— 新增：存放每个 IP 的 20s 中断倒计时定时器 ID
const interruptTimers = reactive({})
// 标记是否已经弹过通讯中断提示（避免多台设备同时中断时重复弹窗）
const hasShownInterruptToast = ref(false)
// 标记是否已经弹过升级中断超时提示（避免多台设备同时超时时重复弹窗）
const hasShownTimeoutToast = ref(false)
// 标记是否已经弹过重连成功提示（避免多台设备同时重连时重复弹窗）
const hasShownReconnectedToast = ref(false)
const showVersions = ref(false)
const isLoadingVersions = ref(false)
const refreshInterval = 2000 // 设置刷新频率为2秒
let versionListener = null // 保存监听器引用
let versionRefreshTimer = null // 保存定时器引用

// 初始化版本监听器（只注册一次）
function initVersionListener() {
  if (versionListener) return // 如果已经注册过，直接返回

  versionListener = (event, Arg) => {
...
const canSendUpdate = computed(() => {
  ...
  return bcuValid && bmuValid && ftpRunning.value // 添加FTP状态检查
})
...
const resetUpgradeProcess = ({ clearInitial = false } = {}) => {
  ...
  for (const ip of Object.keys(errorCodes)) {
    ...
    fileDownloadCompleted[ip] = false // 清理文件下载完成状态
  ...
  // 重置通讯中断提示标志
  hasShownInterruptToast.value = false
  // 重置升级中断超时提示标志
  hasShownTimeoutToast.value = false
  // 重置重连成功提示标志
  hasShownReconnectedToast.value = false
...
async function toggleFtp() {
  if (ftpRunning.value) {
    ...
    // 启动前检查 IP 是否为 10 网段 (192.168.10.xxx)
    const ftpIp = ipstore.selectedInterface
    const ipPattern = /^192\.168\.10\.\d{1,3}$/

    if (!ipPattern.test(ftpIp)) {
      toast.add({
        severity: 'error',
        summary: 'FTP服务器IP地址错误',
        detail: `FTP服务器IP必须为10网段,请在设备连接页选择正确的网卡`,
        life: 5000
      })
      return
    }
...
async function chooseFile() {
  ...
  // 自动设置FTP服务器文件路径为所选升级文件的路径
  if (res.fullPath) {
    // 提取文件所在的目录路径 - 使用浏览器兼容的方式
    const fileDir = res.fullPath.substring(
      0,
      res.fullPath.lastIndexOf('\\') || res.fullPath.lastIndexOf('/')
    )

    // 更新FTP默认目录
    defaultFTPDir.value = fileDir
    window.electron.ipcRenderer.send('set-default-FTP-dir', fileDir)

    toast.add({
      severity: 'success',
      summary: t('upgrade.toast.filePathChanged', { path: fileDir }),
      /*  detail: t('upgrade.toast.ftpPathAutoUpdated'), */
      life: 3000
    })
  }
}
const startUpgradeProcess = () => {
  ...
  // 文件格式校验逻辑
  const fileName = updateFile.value?.toLowerCase() || ''
  const fileExt = fileName.substring(fileName.lastIndexOf('.'))

  // 根据升级类型校验文件格式
  if (selectedUpgrade.value === '0xa001' || selectedUpgrade.value === '0xa002') {
    // BCU或BMU升级，需要.pkg文件
    if (fileExt !== '.pkg') {
      toast.add({
        severity: 'error',
        summary: t('upgrade.toast.fileFormatError'),
        detail: t('upgrade.toast.pkgFileRequired'),
        life: 5000
      })
      return
    }
  } else if (selectedUpgrade.value === '0xa003') {
    // BCU默认参数升级，需要.cfg文件
    if (fileExt !== '.cfg') {
      toast.add({
        severity: 'error',
        summary: t('upgrade.toast.fileFormatError'),
        detail: t('upgrade.toast.cfgFileRequired'),
        life: 5000
      })
      return
    }
...
const registerListener = () => {
  ...
  listenerId.value = (event, Arg) => {
    ...
    if (selectedUpgrade.value === '0xa002') {
      ...
      // 强制升级：0xc0b1(单机强制) 和 0xc0b2(广播强制)
      if (style === '0xc0b1' || style === '0xc0b2') {
        ...
        if (dlFlag && String(dlFlag.row).toLowerCase() === '5bb5') {
          // 如果还没标记为文件下载完成，则提示用户断电重启
          if (!fileDownloadCompleted[deviceIp]) {
            fileDownloadCompleted[deviceIp] = true
            const upgradeTypeKey =
              style === '0xc0b1' ? 'forceSingleUpgrade' : 'forceBroadcastUpgrade'
            const upgradeType = t(`upgrade.toast.${upgradeTypeKey}`)
            toast.add({
              severity: 'info',
              summary: t('upgrade.toast.fileDownloadCompletedSummary', { ip: deviceIp }),
              detail: t('upgrade.toast.fileDownloadCompletedDetail', { upgradeType }),
              life: 15000
            })
            // 重要：不删除pendingIps中的IP，继续监控BMU升级进度
          }
          // 检测到5bb5后，直接返回，等待下一次数据更新再检查BMU升级标志
          // 避免在检测到5bb5的同一时刻立即检查BMU升级标志，导致立即完成升级
          //return
        }

        // 第二阶段：检查BMU升级标志（在文件下载完成后）
        if (fileDownloadCompleted[deviceIp]) {
          ...
          if (bmuFlag) {
            ...
            if (typeof got === 'number') {
              got = got.toString(16).toLowerCase()
            } else {
              got = String(got).toLowerCase().replace(/^0x/, '')
            }
            ...
            // 根据升级类型确定目标完成标志
            const mappingDone = {
              '0xc0b1': '0xb0a3', // 单机强制 -> 强制单机完成
              '0xc0b2': '0xb0a4' // 广播强制 -> 强制广播完成
            }
            const targetDone = mappingDone[style]
            const targetFail = style === '0xc0b1' ? '0xb0b3' : '0xb0b4'

            // 成功：比较时统一格式为 0x + 小写十六进制
            const gotFormatted = `0x${got}`
            console.log(
              `[强制升级] ${deviceIp} BMU升级标志: ${gotFormatted}, 目标完成标志: ${targetDone}, 升级类型: ${style}`
            )
            if (gotFormatted === targetDone) {
              console.log(`[强制升级] ${deviceIp} 检测到升级完成标志: ${gotFormatted}`)
              toast.add({
                severity: 'success',
                summary:
            ...
            // 失败：比较时统一格式为 0x + 小写十六进制
            if (gotFormatted === targetFail) {
              const bmuError = errorGroup.element.find((e) => e.label === 'BMU升级故障码')
              const errorMsg = bmuError && bmuError.value !== '无故障' ? bmuError.value : '升级失败'
              errorCodes[deviceIp] = errorMsg

              toast.add({
                severity: 'error',
                summary: `${deviceIp}` + t('upgrade.toast.infoBMUUpgradeFailed'),
                detail: t(`upgrade.toast.errorBMUUpgradeFailed.${errorMsg}`) || errorMsg,
                life: 6000
              })
              pendingIps.delete(deviceIp)
        ...
        // 如果文件下载已完成但BMU升级标志未变化，继续监控
        return
      }
      // —— 常规 BMU 升级（0xc0a1 / 0xc0a2）：同样记录下载完成标志，避免重连后重复下发 ——
      const dlFlagNormal = errorGroup.element.find((e) => e.label === '升级文件下载完成标志')
      if (dlFlagNormal && String(dlFlagNormal.row).toLowerCase() === '5bb5') {
        fileDownloadCompleted[deviceIp] = true
      }
      const bmuFlag = errorGroup.element.find((e) => e.label === 'BMU升级标志')
      if (!bmuFlag) return
...
const sendResetCommand = async () => {
  ...
  // ⭐ 新增：立即恢复所有未升级IP的数据读取
  const allConnectedIps = connectedIps.value
  const notUpgradedIps = allConnectedIps.filter((ip) => !ipListUpdate.value.includes(ip))
  console.log('未升级的IP，立即恢复读取:', notUpgradedIps)
  notUpgradedIps.forEach((ip) => {
    console.log(`[恢复读取] ${ip} 未升级，立即恢复数据读取`)
    window.electron.ipcRenderer.send('start-reading-data', {
      action: 'start',
      targetIp: ip
    })
            ...
            detail:
              locale.value === 'zh'
                ...
                : (te(`upgrade.toast.errorBCUUpgradeFailed.${stillError}`)
                    ...
                    : stillError) + ', please wait for retry',
            life: 4000
          })
...
function handleConnectionInterrupted(event, { ip, success, error }) {
  ...
  if (
    ...
    // 使用布尔标记记录该 IP 已中断，供重连时判断
    interruptTimers[ip] = true
    // 只在第一次检测到中断时弹窗，避免多台设备同时中断时重复弹窗
    if (!hasShownInterruptToast.value) {
      hasShownInterruptToast.value = true
      toast.add({
        severity: 'warn',
    ...
    // 删除40s超时终止逻辑：改为依赖全局 UPGRADE_TIMEOUT_MS 定时器统一结束升级
    // 不再为每个设备启动独立的中断超时定时器，等待重连成功后在 handleConnectionResumed 中续传
  }
}
function handleConnectionResumed(event, { ip, success, error }) {
  ...
  if (reconnected && interruptTimers[ip]) {
    ...
    if (!hasShownReconnectedToast.value) {
      hasShownReconnectedToast.value = true
      toast.add({
        severity: 'success',
    ...
    // —— 新增：重连后，按下载是否完成决定是否重发升级指令 ——
    const inSending = upgradeState.value === UPGRADE_STATE.SENDING_INSTRUCTIONS
    const isPending = pendingIps.has(ip)
    const downloadedBCU = !!bcuDownloadStatus[ip]
    const downloadedBMU = !!fileDownloadCompleted[ip]
    const shouldResend = inSending && isPending && !downloadedBCU && !downloadedBMU
...
onMounted(async () => {
  ...
  loadVersions() // 初次加载数据
  // 设置定时器，保存引用以便后续清理
  versionRefreshTimer = setInterval(() => {
...
onBeforeUnmount(() => {
  ...
  /* window.electron.ipcRenderer.removeAllListeners('connection-status') */
})
