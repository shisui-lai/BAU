...
// 原始hex数据
const hexData =
  ''.replace(
    /\s+/g,
    ''
  )
...
function getBrokenwireDisplayValue(label, value, originalClass) {
  ...
  const collectionOfflineFields = ['电压采集状态', '温度采集状态', '电压二级掉线', '温度二级掉线']

  // 检查是否为采集掉线类型
  const isCollectionOffline = collectionOfflineFields.some(
    (field) => originalClass.includes(field) || label.includes(field)
  )
  if (isCollectionOffline) {
...
function analyzeField(bitValue, label, dataClass) {
  ...
  let schemaValue
  if (
    label.includes('电压一级掉线') ||
    label.includes('温度一级掉线') ||
    label.includes('电压二级掉线') ||
    label.includes('温度二级掉线')
  ) {
...
for (let i = 0; i < 8; i++) {
  ...
  console.log(
    `  ${label}: 位${bitValue} → Schema"${result.schemaValue}" → 显示"${result.displayValue}" → 故障${result.isFault}`
  )
}
offset += 2
...
for (let bit = 0; bit < 12; bit++) {
  ...
  if (bit < 8) {
    // 只显示前8个
    console.log(
      `  Cell${bit + 1}: 位${bitValue} → Schema"${result.schemaValue}" → 显示"${result.displayValue}" → 故障${result.isFault}`
    )
  }
}
offset += 2
